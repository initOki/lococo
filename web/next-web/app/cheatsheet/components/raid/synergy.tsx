...
import { synergyData } from './synergy-data';

const Synergy = () => {
  const [searchText, setSearchText] = useState('');
  const [searchResult, setSearchResult] = useState<any[]>([]);

  const returnSynergy = (synergy: any) => {
    ...
    <div>
      <div className="flex items-end">
        ...
        <button
          ...
          onClick={() => findSynergy(searchText)}
          style={{ padding: '9px' }}
        >
          검색
        </button>
            ...
            <tr className={searchResult.includes('critical chance') ? 'isSearched' : ''}>
              ...
              <td>
                {returnSynergy('critical chance')?.always.map((item) => {
                  return <span className={`mr-[5px] ${highlight(item.name) ? 'isHightlight' : ''}`}>{item.name}</span>;
                })}
              </td>
              <td>
